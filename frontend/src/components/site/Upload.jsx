...
export default function Upload() {
  ...
  const handleFile = (file) => {
    ...
    setTimeout(() => {
      ...
      setResult(generate());
      setPhase("done");
      toast.success("Your Life Index is ready.");
    }, 2900);
  };
  ...
  return (
    <section id="upload" className="relative paper grain torn-bottom py-28 md:py-40" data-testid="upload-section">
      <div className="relative z-10 max-w-[1300px] mx-auto px-6 md:px-16">
        <div className="font-mono text-[11px] tracking-label text-[color:var(--rust)] mb-4">FILE 005 — SUBMIT YOUR DENIM</div>
        <h2 className="font-serif-display text-[color:var(--ink)] text-4xl md:text-6xl mb-4 max-w-3xl leading-[1.05]">
          Generate your Life Index.
        </h2>
        <p className="font-mono text-xs text-[color:var(--ink)]/60 mb-14 max-w-md">
          Upload a photo of your own worn denim. We read the evidence and return your Archive Entry.
        </p>
        ...
        <AnimatePresence mode="wait">
          {phase === "idle" && (
            ...
            >
              <div className="wax-seal w-14 h-14 mb-6" />
              <p className="font-serif-display text-2xl md:text-3xl text-[color:var(--ink)] mb-2">
                Drag & drop your denim photo, or click to browse.
              </p>
          ...
          {phase === "reading" && (
            <motion.div
              key="reading"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="hairline py-24 px-6 flex flex-col items-center text-center"
            ...
            >
              ...
              <div className="space-y-2">
                {STEPS.map((s, i) => (
                  <div key={s} className={`font-serif-display text-xl md:text-2xl transition-opacity duration-500 ${i <= step ? "text-[color:var(--ink)] opacity-100" : "text-[color:var(--ink)] opacity-25"}`}>
                    {s}
                  </div>
                ))}
              </div>
          ...
          {phase === "done" && result && (
            ...
            >
              <div>
                ...
                <h3 className="font-serif-display text-[color:var(--ink)] text-3xl md:text-5xl mb-6 leading-tight">
                  The evidence has been read.
                </h3>
                <p className="font-mono text-xs text-[color:var(--ink)]/60 leading-relaxed mb-8 max-w-sm">
                  Your denim now has an Archive Entry. Drag the card to flip it and read your full Life Index, then download your Evidence Card to keep or share.
                </p>
                <button className="btn-stamp" onClick={reset} data-cursor="reset" data-testid="upload-reset-btn">
                  Submit Another
                </button>
              </div>
              <div className="flex justify-center">
                <ArchiveCard data={result} photoUrl={photo} />
              </div>
            </motion.div>
          )}
        </AnimatePresence>
