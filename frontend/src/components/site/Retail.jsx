  ...
  return (
    <section className="relative paper grain py-28 md:py-40" data-testid="retail-section">
      <div className="relative z-10 max-w-[1500px] mx-auto px-6 md:px-16">
        ...
        <h2 className="font-serif-display text-[color:var(--ink)] text-4xl md:text-6xl mb-16 max-w-2xl leading-[1.02]">The Evidence Station.</h2>

        <div className="grid lg:grid-cols-[1.4fr_1fr] gap-8">
          <motion.div className="relative" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.9 }}>
            <Tape className="absolute -top-4 right-10 z-20" rotate={4} text="POP-UP · 2025" />
            <div className="hairline p-3 bg-[color:var(--kraft-2)]" data-cursor="inspect">
              <AssetImage asset={ASSETS.storePhoto} kraft className="aspect-[16/10]" note="Pop-up interior: kiosk, shelves, mirror" />
            </div>
          </motion.div>
          ...
          <div className="flex flex-col gap-6">
            <div className="hairline p-6 bg-[color:var(--ink)] text-[color:var(--kraft)]" data-cursor="scan">
              ...
              <div className="grid grid-cols-6 gap-1 w-24 h-24 mb-4" aria-hidden>
                {Array.from({ length: 36 }).map((_, i) => (<span key={i} style={{ background: (i * 7 + (i % 5)) % 3 === 0 ? "var(--kraft)" : "transparent" }} />))}
              </div>
